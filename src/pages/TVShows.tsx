...
const TVShows = () => {
  ...
  const fetchTVShows = async (page: number) => {
    try {
      setLoading(true);
      const response = await fetch(`https://api.themoviedb.org/3/discover/tv?api_key=${TMDB_API_KEY}&language=en-US&page=${page}&vote_count.gte=50`);
      const data: TVResponse = await response.json();
      setTVShows(data.results.slice(0, 24));
    ...
    <div className="min-h-screen bg-gray-900">
      ...
      <main className="container mx-auto px-4">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-bold text-white">All TV Shows</h2>
        </div>
        ...
        {loading ? (
          ...
          <>
            <MovieGrid movies={tvShows as any} type="tv" />
            {renderPagination()}
          </>
        )}
      </main>
